...
//--------------------------------------------------------------------------------------------------------
	...
	NSArray * vdeInfosPhotoAAfficher;
	vdeInfosPhotoAAfficher = [self.vdeViewControllerImagier vdeDemandePhotosAAfficher:(int) sender.value];

	[self vdeAffichePhoto: vdeInfosPhotoAAfficher];
}
...
-(void) vdeAffichePhoto: (NSArray *) infosPhoto {
//--------------------------------------------------------------------------------------------------------
	
	vdePhotoAAfficher							= infosPhoto[0];
	vdeVueImageAInclureDansScrollView.image		= vdePhotoAAfficher;// si j'utilisais un dicitonnaire, se serait plus clair...
	VDEPhotoPourAfficher * vdePhotoInfo			= infosPhoto[1];
	
	vdeSliderLargeur.value					= (int)((vdePhotoInfo.vdeValeursZoom.width)*100);
	vdeSliderHauteur.value					= (int)((vdePhotoInfo.vdeValeursZoom.height)*100);
	vdeScrollViewZoneZoomPhoto.zoomScale	= vdePhotoInfo.vdeValeursZoom.width/100;
	// pour synchro avec méthode zoom pinch // ( width = height )
	...
	vdeLabelNomPhoto.text = [NSString stringWithFormat:@"%@",vdePhotoInfo.vdeNomPhotoSource];
